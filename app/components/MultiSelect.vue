...
<template>
  <DefineLeadingTrailing>
    <span v-if="isLeading || !!slots.leading" :class="classes.leading({ class: props.ui?.leading })">
      <slot name="leading" :model-value="modelValue" :open="open" :ui="ui">
        <UIcon v-if="isLeading && leadingIconName" :name="leadingIconName"
          :class="classes.leadingIcon({ class: props.ui?.leadingIcon })" />
      </slot>
    </span>

    <component :is="props.multiple && !props.checkbox ? ComboboxTrigger : 'span'" v-if="isTrailing || !!slots.trailing"
      :class="classes.trailing({ class: props.ui?.trailing })">
      <slot name="trailing" :model-value="modelValue" :open="open" :ui="ui">
        <UIcon v-if="trailingIconName" :name="trailingIconName"
          :class="classes.trailingIcon({ class: props.ui?.trailingIcon })" />
      </slot>
    </component>
  </DefineLeadingTrailing>

  <ComboboxRoot :id="id" ref="comboboxRootRef" v-model="selected" v-model:open="open" :name="name"
    :class="classes.root({ class: [props.ui?.root, props?.class] })" ignore-filter :required="required"
    :as-child="!!props.multiple || (props.multiple && !props.checkbox)" :multiple="props.multiple"
    :reset-search-term-on-blur="false" :reset-search-term-on-select="false" :by="(a, b) => props.checkSelection(a, b)"
    :disabled="isDisabled" @update:model-value="onUpdate" @update:open="onUpdateOpen" @keydown.enter.prevent>
    <ComboboxAnchor :as-child="!props.multiple || (props.multiple && props.checkbox)"
      :class="classes.base({ class: props.ui?.base })">
      <TagsInputRoot v-if="props.multiple && !props.checkbox" v-slot="{ modelValue: tags }"
        :model-value="(selected as MultiSelectItem[])" :disabled="isDisabled" delimiter="" as-child
        @remove-tag="onRemoveTag">
        <TagsInputItem v-for="(item, index) in (tags as MultiSelectItem[])" :key="index" :value="item"
          ...
          <TagsInputItemDelete :class="classes.tagsItemDelete({ class: props.ui?.tagsItemDelete })"
            :disabled="disabled">
            <slot name="tags-item-delete" :item="item" :index="index">
              <UIcon :name="props.deleteIcon"
        ...
        <div :class="classes.tagsInput({ class: props.ui?.tagsInput })">
          <ComboboxInput v-model="search" as-child @focus="onFocus" @blur="onBlur" @update:model-value="onInput">
            <ComboboxTrigger as-child>
              <TagsInputInput v-bind="{ ...$attrs, ...ariaAttrs }" type="text" :tabindex="null"
                :placeholder="placeholder" :class="classes.tagsInputInput({ class: props.ui?.tagsInputInput })"
                :data-test="dataTest ?? name" @keydown.enter.prevent />
            </ComboboxTrigger>
          </ComboboxInput>

          <span v-if="typeof props.tagsInputIcon === 'string' || !!slots['tags-input-icon']"
            :class="classes.tagsInputLeading({ class: props.ui?.tagsInputLeading })">
            <slot name="tags-input-icon" :open="open" :ui="ui">
              <UIcon v-if="typeof props.tagsInputIcon === 'string'" :name="props.tagsInputIcon"
      ...
      <ComboboxTrigger v-else :class="classes.base({ class: props.ui?.base })" :tabindex="0"
        :data-test="dataTest ?? name" @keydown.up.down.enter.prevent="onOpenContent">
        <slot :model-value="selected" :open="open">
          <span v-if="displayValue" :class="classes.value({ class: props.ui?.value })">
            {{ displayValue }}
          </span>
          <span v-if="!displayValue" :class="classes.placeholder({ class: props.ui?.placeholder })">
            {{ placeholder ?? '&nbsp;' }}
          </span>
        </slot>
    ...
    <ComboboxPortal v-bind="portalProps">
      <ComboboxContent :class="classes.content({ class: props.ui?.content })" v-bind="contentProps">
        <ComboboxInput v-if="!!props.searchInput || (props.multiple && props.checkbox && !!props.searchInput)"
          v-model="search" :display-value="() => search" as-child>
          <UInput type="text" autofocus autocomplete="off" v-bind="searchInputProps"
            :class="classes.searchInput({ class: props.ui?.searchInput })"
            @update:model-value="(val) => onInput(String(val))" />
        </ComboboxInput>

        <div :class="classes.viewport({ class: props.ui?.viewport })" class="overscroll-contain" role="presentation">
          <ComboboxGroup v-if="props.multiple && props.checkbox && props.checkAll && options.length > 0"
            key="group-checkbox" :class="classes.group({ class: props.ui?.group })">
            <ComboboxItem :class="classes.item({ class: props.ui?.item })"
              :value="{ label: props.checkboxLabel, value: 'all' }" :data-checked="isCheckedAll"
              @select.prevent="onCheckAll">
              <span :class="classes.itemLeading({ class: props.ui?.itemLeading })">
                <UCheckbox :model-value="isCheckedAll" :size="props.checkboxSize" />
              </span>
          ...
          <ComboboxGroup v-for="(group, groupIndex) in options" :key="`group-${groupIndex}`"
            :class="classes.group({ class: props.ui?.group })">
            <ComboboxSeparator v-if="groupIndex !== 0 && hasChildren"
              :class="classes.separator({ class: props.ui?.separator })" />

            <ComboboxLabel v-if="hasChildren" :class="classes.label({ class: props.ui?.label })">
              {{ group.label }}
            </ComboboxLabel>

            <ComboboxItem v-for="(item, itemIndex) in toArray<MultiSelectItem>(group.children!)"
              :key="`item-${itemIndex}`" :value="item" :class="classes.item({ class: props.ui?.item })"
              :disabled="item.disabled" :data-test="dataTest ? `${dataTest}-${item.value}` : undefined"
              @select="onSelect($event, item)">
              <slot name="item" :item="item" :group-index="groupIndex" :index="itemIndex">
                <span v-if="!!slots['item-leading'] || (props.multiple && props.checkbox)"
                  :class="classes.itemLeading({ class: props.ui?.itemLeading })">
                  <slot name="item-leading" :item="item" :open="open" :group-index="groupIndex" :index="itemIndex">
                    <UCheckbox v-if="props.multiple && props.checkbox"
                ...
                <span :class="classes.itemLabel({ class: props.ui?.itemLabel })">
                  <slot name="item-label" :item="item" :open="open" :group-index="groupIndex" :index="itemIndex">
                    {{ item.label }}
                  </slot>
                </span>

                <span v-if="!(props.multiple && props.checkbox)"
                  :class="classes.itemTrailing({ class: props.ui?.itemTrailing })">
                  <slot name="item-trailing" :item="item" :open="open" :group-index="groupIndex" :index="itemIndex" />

                  <ComboboxItemIndicator as-child>
                    <UIcon :name="props.selectedIcon"
          ...
          <ComboboxEmpty v-if="!isLoading" :class="classes.empty({ class: props.ui?.empty })">
            <slot name="empty" :search-term="search">
              {{ props.empty }}
            </slot>
          </ComboboxEmpty>
          ...
          <slot v-if="isLoading" name="loading">
            <div :class="classes.loading({ class: props.ui?.loading })">
              <div :class="classes.loadingSpinner({ class: props.ui?.loadingSpinner })">
                <UIcon :name="props.loadingIcon" :class="classes.loadingIcon({ class: props.ui?.loadingIcon })" />
              </div>
          ...
          <div v-intersection-observer="onIntersectionBottom" class="w-full h-0.5" />
        </div>
      </ComboboxContent>
    </ComboboxPortal>
  </ComboboxRoot>
