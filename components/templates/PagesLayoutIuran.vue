  ...
  <OrganismFilterTable :filter="filterLayout" v-model:params="params" v-model:open-modal="showModal"
    ...
    <template #modal-header>
      <div class="flex items-center justify-between mb-5">
        <h5 class="font-bold text-lg">Add contribution</h5>
    ...
    <template #modal-body>
      <div>
        <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
          ...
          <UFormField label="admission" name="userId">
            <USelect v-model="state.admission" :items="admission" class="w-full" />
          </UFormField>
          <UFormField label="Uang" name="money">
            <UInput v-model="state.money" class="w-full" />
          </UFormField>
...
const schema = z.object({
  ...
  admission: z.string().nonempty(),
  money: z.string().nonempty(),
})
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  const { execute, error } = await useFetch('/api/v1/add-iuran', {
    method: 'POST',
    body: {
